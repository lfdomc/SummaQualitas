-- Limpiar y corregir todas las funciones optimizadas

-- Eliminar TODAS las versiones de las funciones conflictivas
DROP FUNCTION IF EXISTS get_expenses_paginated(INTEGER, INTEGER, UUID, TEXT, TEXT) CASCADE;
DROP FUNCTION IF EXISTS get_expenses_paginated(UUID, VARCHAR, VARCHAR, INTEGER, INTEGER, VARCHAR, VARCHAR) CASCADE;
DROP FUNCTION IF EXISTS get_projects_with_summary(INTEGER, INTEGER, TEXT) CASCADE;
DROP FUNCTION IF EXISTS get_incomes_with_project_info(INTEGER, INTEGER, UUID, TEXT) CASCADE;
DROP FUNCTION IF EXISTS get_incomes_with_project_info(UUID, VARCHAR, INTEGER, INTEGER) CASCADE;
DROP FUNCTION IF EXISTS search_expenses_fulltext(TEXT, INTEGER, INTEGER, UUID) CASCADE;
DROP FUNCTION IF EXISTS get_expenses_by_category_period(DATE, DATE, UUID) CASCADE;
...
-- 2. Función para obtener gastos paginados
...
-- 3. Función para obtener proyectos con resumen
CREATE FUNCTION get_projects_with_summary(
    p_limit INTEGER DEFAULT 20,
    p_offset INTEGER DEFAULT 0,
...
-- 4. Función para obtener ingresos con información del proyecto
CREATE FUNCTION get_incomes_with_project_info(
    p_limit INTEGER DEFAULT 20,
    p_offset INTEGER DEFAULT 0,
    p_project_id UUID DEFAULT NULL,
    p_status TEXT DEFAULT NULL
...
-- 5. Función de búsqueda de texto completo en gastos
CREATE FUNCTION search_expenses_fulltext(
    p_search_term TEXT,
    p_limit INTEGER DEFAULT 20,
    p_offset INTEGER DEFAULT 0,
    p_project_id UUID DEFAULT NULL
...
BEGIN
    ...
    SELECT 
        e.id,
        e.project_id,
        COALESCE(s.name, '')::TEXT as supplier_name,
        e.description,
        e.amount,
        e.category::TEXT,
        e.expense_date,
...
-- 6. Función para obtener gastos por categoría y período
CREATE FUNCTION get_expenses_by_category_period(
    p_start_date DATE,
    p_end_date DATE,
    p_project_id UUID DEFAULT NULL
)
